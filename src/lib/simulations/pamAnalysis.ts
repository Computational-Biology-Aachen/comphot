...
export function interpolateAtIndices(
	...
	if (method === 'akima') {
		...
		for (let i = 0; i < n; i++) {
			...
			const denom = w1 + w2;
			t.push(denom < 1e-14 ? (mExt[i + 1] + mExt[i + 2]) / 2 : (w1 * mExt[i + 1] + w2 * mExt[i + 2]) / denom);
		}
		for (let p = 0; p < n - 1; p++) {
			const x0 = indices[p],
				x1 = indices[p + 1];
...
export function computeNpq(fluoNorm: number[], peakIndices: number[]): number[] {
	if (peakIndices.length === 0) return [];
	const Fm0 = fluoNorm[peakIndices[0]];
	return peakIndices.map((i) => (Fm0 - fluoNorm[i]) / fluoNorm[i]);
}

export function computePhiPsii(fluoNorm: number[], peakIndices: number[]): number[] {
