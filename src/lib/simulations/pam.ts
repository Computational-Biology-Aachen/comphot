...
export function buildPamProtocol(params: PamParams): ProtocolStep[] {
	const { lightIntensity, saturatingPulse, totalMinutes, darkLength, pulseInterval } = params;
...
export function buildMemoryProtocol(params: MemoryPamParams): ProtocolStep[] {
	...
	if (relaxationLength > 0) {
		...
		simulatePeriod(relaxStart, relaxPeriod1End - relaxStart, dark, lightIntensity, true);
	...
	const totalEnd = darkLength + trainingLength + relaxationLength + memoryLength;
	if (t < totalEnd) {
