...
def _npq_ode(_t, y, pars, PFD):
    ...
    dP_dt  = vps2 - vPQox
    dH_dt  = (2.0 / bH) * vps2 - (4.0 / bH) * vPQox - (14.0 / 3.0) / bH * vATPsynthase - (1.0 / bH) * vLeak
    dE_dt  = vATPactivity
    ...
    def integrate(self, protocol, y0_list, pars_override_dict):
        ...
        if pars_override_dict:
            try:
                ...
                items = pars_override_dict.items() if hasattr(pars_override_dict, 'items') else pars_override_dict.to_py().items()
                for k, v in items:
                    pars[k] = float(v)
            except Exception:
                pass
        ...
        try:
            proto_py = protocol.to_py() if hasattr(protocol, 'to_py') else list(protocol)
        except Exception:
        ...
        for step in proto_py:
            ...
            try:
                ...
                for i in range(y_arr.shape[1]):
                    ...
                    fluo_arr.append(_fluorescence(B0_i, B2_i, Q_i, pars["kH"], pars["kF"], pars["kP"]))
